...
Your code:
```{r,echo=TRUE}
fit_alive <- lm(SURVIVED_bin ~ poly(TOBACCO, 2), data = Train_COVID)

pred_fit <- predict(fit_alive, newdata = Test_COVID, se = TRUE)
actual <- Test_COVID$SURVIVED_bin
calc_loss(pred_fit$fit, actual)
```
...
```{r}
fwd = regsubsets(Outstate~., data=college_train, nvmax=17, method="forward")
...
```{r}
par(mfrow=c(2,2))
plot(1:17, fwd_summary$cp,xlab="Variables",ylab="Cp",main="Cp", type='b')
plot(1:17, fwd_summary$bic,xlab="Variables",ylab="BIC",main="BIC", type='b')
plot(1:17, fwd_summary$adjr2,xlab="Variables",ylab="Adjusted R2",main="Adjusted R2", type='b')
```
...
Your code:
```{r}
plot(college_train$Room.Board)
plot(college_train$PhD)
plot(college_train$perc.alumni)
plot(college_train$Expend)
```

```{r,echo=TRUE}
gam.m1 = gam(Outstate ~ Private + s(Room.Board,3) + s(PhD,2) + s(perc.alumni,2) + s(Expend,2), data=college_train)
summary(gam.m1)
```
...
~~~
Expend shows a strong sigh of non-linear relationship. The Npar Df is 1, but the p-value is very small.
...
poly.1 = lm(mpg ~ weight ,data=Auto)
poly.2 = lm(mpg ~ poly(weight ,2) ,data=Auto)
poly.3 = lm(mpg ~ poly(weight ,3) ,data=Auto)
poly.4 = lm(mpg ~ poly(weight ,4) ,data=Auto)
poly.5 = lm(mpg ~ poly(weight ,5) ,data=Auto)
poly.6 = lm(mpg ~ poly(weight ,6) ,data=Auto)
